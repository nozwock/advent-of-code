...
// One advent of code part 2 problem where I could reuse most of the code from part 1 MUAHAHAAHA
...
fn main() {
    ...
    let total_boards = bingo_boards.len();
    let mut boards_won = HashMap::new();
    let mut last_win_stats = None;

    // MUAhUAHAAHHA; I've done it
    'the_holy_loop_II: for draw in &draws {
        for (idx, board) in bingo_boards.iter_mut().enumerate() {
            mark_board(board, draw);
            if is_winner(board) {
                boards_won.entry(idx).or_insert(true);
                if boards_won.len() == total_boards {
                    last_win_stats = Some((idx, draw));
                    break 'the_holy_loop_II;
                }
            }
        }
    }

    let sum = sum_unmarked(&bingo_boards[last_win_stats.unwrap().0]);
    println!(
        "board `{}` won last with a total score of: {}",
        last_win_stats.unwrap().0 + 1,
        last_win_stats.unwrap().1 * sum
    );
}
